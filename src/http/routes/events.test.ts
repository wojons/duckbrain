...
describe('POST /api/events/:namespace/broadcast', () => {
  it('should broadcast event and return success', async () => {
    const app = createApp();
    const { status, body } = await jsonRequest(app, 'POST', '/api/events/testns/broadcast', {
      type: 'memory.created',
  ...
  it('should include timestamp in broadcast response', async () => {
    const app = createApp();
    const { status, body } = await jsonRequest(app, 'POST', '/api/events/ns1/broadcast', {
      type: 'custom.event',
      data: { payload: 'test' },
    });
...
describe('GET /api/events/:namespace/stats', () => {
  it('should return connection stats for a namespace', async () => {
    const app = createApp();
    const { status, body } = await jsonRequest(app, 'GET', '/api/events/myns/stats');
  ...
  it('should have activeConnections as a number', async () => {
    const app = createApp();
    const { status, body } = await jsonRequest(app, 'GET', '/api/events/ns-x/stats');
  ...
  it('should include allNamespaces array with connection counts', async () => {
    const app = createApp();
    const { status, body } = await jsonRequest(app, 'GET', '/api/events/all/stats');
