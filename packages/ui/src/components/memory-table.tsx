...
export function MemoryTable({ namespace }: MemoryTableProps) {
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, memory: MemoryResponse) => {
      e.preventDefault();
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <table className="w-full">
          <thead className="sticky top-0 z-10">
            {table
              .getHeaderGroups()
              .map((headerGroup: HeaderGroup<MemoryResponse>) => (
                <tr
                  key={headerGroup.id}
                  className="border-b"
                ...
                >
                  {headerGroup.headers.map(
                    (header: Header<MemoryResponse, unknown>) => {
            ...
            {virtualizer.getVirtualItems().map((virtualRow: VirtualItem) => {
              ...
              return (
                ...
                >
                  {row
                    .getVisibleCells()
                    .map((cell: Cell<MemoryResponse, unknown>) => (
                      <td
                        key={cell.id}
      ...
      <ContextMenu
        items={
          contextMenu
            ? createMemoryContextMenuItems(
                {
                  id: contextMenu.memory.id,
                  key: contextMenu.memory.key,
                  content: contextMenu.memory.content,
                },
                {
