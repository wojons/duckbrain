...
describe("Docker Build Integration", () => {
  it("should build the production Docker image", () => {
    const result = run("docker build -t duckbrain-int-test .", {
      cwd: process.cwd(),
    });
  ...
  it("should have git installed in the container", () => {
    const result = run(
      'docker run --rm --entrypoint="" duckbrain-int-test which git',
    );
  ...
  it("should have node installed in the container", () => {
    const result = run(
      'docker run --rm --entrypoint="" duckbrain-int-test node --version',
    );
  ...
  it("should have /data directory with proper ownership", () => {
    const result = run(
      'docker run --rm --entrypoint="" duckbrain-int-test ls -ld /data',
    );
  ...
  it("should have the entrypoint script", () => {
    const result = run(
      'docker run --rm --entrypoint="" duckbrain-int-test ls -la /app/scripts/docker-entrypoint.sh',
    );
  ...
  it("should start HTTP server and respond to health check", async () => {
    ...
    run(
      `docker run -d --name ${containerName} -p 127.0.0.1:${port}:3000 -e DUCKBRAIN_DATA_DIR=/data duckbrain-int-test`,
    );
